...
def closestServers(client, all=False):
    ...
    urls = [
        '://www.speedtest.net/speedtest-servers-static.php', # Verificar se nesta lista de servers .php precisa ser um utilizado um .br ou .pt.
    ...
    if not servers:
        print_('Failed to retrieve list of speedtest.net servers:\n\n %s' %
               '\n'.join(errors))
        sys.exit(1)
        return  # RETORNO ADICIONADO PARA TESTE. EXCLUIR, CASO DÊ PROBLEMA!
...
#     signal.signal(signal.SIGINT, ctrl_c) 
    ...
    downSpeed = ('%0.2f M%s/s' %((dlspeed / 1000 / 1000) * args.units[1], args.units[0]))
    downSpeed2 = ('%0.2f' %(dlspeed / 1000 / 1000) * args.units[1])
    speedtest.Down = ('Download: %0.2f M%s/s' %((dlspeed / 1000 / 1000) * args.units[1], args.units[0]))
    ...
    upSpeed = ('%0.2f M%s/s' % ((ulspeed / 1000 / 1000) * args.units[1], args.units[0]))
    upSpeed2 = ('%0.2f' %(ulspeed / 1000 / 1000) * args.units[1])
    speedtest.Up = ('Upload: %0.2f M%s/s' % ((ulspeed / 1000 / 1000) * args.units[1], args.units[0]))
