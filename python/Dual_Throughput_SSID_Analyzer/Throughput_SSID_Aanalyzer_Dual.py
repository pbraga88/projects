...
#         root.geometry("1080x720+0+0") # width x height + x_offset + y_offset:
        ...
        self.profile = Label(master, text="SSID2:", font=self.fonte, bg = "ghost white")
    ...
    def Start(self):
        ...
        worksheet.add_table('A1:E1000', {'header_row':True,'banded_rows': True, 'columns':[{'header':'Time'},
                                                                                         {'header': 'Download (Mbit/s) N1'},
                                                                                         {'header': 'Upload (Mbit/s) N1'},
                                                                                         {'header': 'Download (Mbit/s) N2'},
                                                                                         {'header': 'Upload (Mbit/s) N2'},
                                                                                         ]})
        while True:
            self.controlBit = 0 # Bit de Controle (Initialize)
            # self.networkControlBit = 0 # Bit de controle da conexão
            try:
                timeToWait = 300 # int(self.profileEntry.get())
                if timeToWait < 30:
                ...
                ''' NETWORK 2!!!!!!!! '''
                self.checkConnection = str(check_output("netsh wlan connect name=%s" % (self.profileEntry.get())))
                print('Starting second test!')
                time.sleep(30)

                if self.controlBit == 1:
                    self.networkControlBit = 1
                    link2 = speedTest.speedtest()
                    ...
                    # worksheet.write(x,z, time.ctime())
                    worksheet.write(x,z+3, lista[0])
                    worksheet.write(x,z+4, lista[1])
                    x+=1
                    z=0
                    lista = list()
                else:
                    ...
                    self.log3.insert(INSERT, 'Fail')
                    # worksheet.write(x,z, time.ctime())
                    worksheet.write(x,z+3, 'Fail')
                    worksheet.write(x,z+4, 'Fail')
                    x+=1
                    z=0
